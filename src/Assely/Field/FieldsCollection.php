...
class FieldsCollection implements JsonSerializable
{
    ...
    {
        if ($field->isTypeOf('tinymce')) {
            throw new FieldException("Tinymce [{$field->getSlug()}] field cannot be nested in repeatable field.");
        }
    ...
    public function getSanitizedValues(&$values = [])
    {
        array_walk($this->fields, function ($field, $index) use (&$values) {
            if (is_array($field)) {
                return $this->sanitizeConditionalValues($values, $field, $index);
            }

            return $this->sanitizeValues($values[$field->getSlug()], $field);
        });
